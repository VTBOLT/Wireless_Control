...
void checkCan(MessageData1 *messageData, MCP_CAN canObj)
{
    if (!digitalRead(CAN0_INT)) // If CAN0_INT pin is low, read receive buffer
    {
        ...
        case AUX_BATTERY:
            ...
            break;
        case MAIN_BATTERY:
            messageData->pack_state_of_charge = rxBuf[0];
            break;
        case MAIN_PACK_TEMP:
            messageData->high_cell_temp = rxBuf[0];
            messageData->low_cell_temp = rxBuf[1];
            break;
        case MOTOR_TEMP:
            messageData->motor_temperature = rxBuf[0] << 8;
            messageData->motor_temperature += rxBuf[1];
            break;
        case BMS_TEMP:
            messageData->bms_temperature = rxBuf[0] << 8;
            messageData->bms_temperature += rxBuf[1];
            break;
        case RPM:
            messageData->motor_speed = rxBuf[0] << 8;
            messageData->motor_speed += rxBuf[1];
            break;
        case SPEED:
            messageData->bike_speed = rxBuf[0] << 8;
            messageData->bike_speed += rxBuf[1];
            break;
        default:
            break;
        }
